...
public class AlbumDAO {
    ...
    public boolean agregarAlbum(Album album) {
        ...
        try {
            ...
            con = Conexion.getConnection();
            ps = con.prepareStatement("INSERT INTO album_usuario(id_album,name,privacidad,id_usuario)"
                    + " VALUES(?,?,?,?)");

            ps.setInt(1, album.getIdAlbum());
            ps.setString(2, album.getNombre());
            ps.setString(3, album.getPrivacidad());
            ps.setInt(4, album.getIdUsuario());
            ps.executeUpdate();
            resultado = true;
            //  con.commit();
